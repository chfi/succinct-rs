...
impl<Block: BlockType> VectorBase<Block> {
    ...
    pub fn with_capacity(element_bits: usize, capacity: u64) -> Self {
        Self::block_with_capacity(Block::ceil_div_nbits(capacity * element_bits as u64))
    }
    ...
    fn additional_blocks(&self, element_bits: usize, additional: u64) -> usize {
        Block::ceil_div_nbits((self.len + additional) * element_bits as u64)
...
impl<'a, Block: BlockType> Iterator for Iter<'a, Block> {
    ...
    fn nth(&mut self, n: usize) -> Option<Self::Item> {
        self.start = self.start + n as u64;
        self.next()
    }
}
